...
export default function AdminUserManagement({ token }) {
  ...
  // --- VRAIE BDD POUR LES CLUBS ---
  const [clubs, setClubs] = useState([]);
  const [editingClub, setEditingClub] = useState(null); 

  // 1. Fetch Users & Clubs depuis MongoDB
  ...
  const handleImageUpload = (e, callback) => {
    ...
    if (file) {
      if (file.size > 2000000) return toast.error("Image trop lourde (Max 2MB)");
  ...
  // Composant Upload Photo Réutilisable
    ...
    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px'}}>
      <div style={{width: '70px', height: '70px', borderRadius: '50%', background: '#e2e8f0', position: 'relative', overflow: 'hidden', border: '2px solid #cbd5e1'}}>
        ...
        <label style={{position: 'absolute', bottom: 0, left: 0, right: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', padding: '4px', cursor: 'pointer'}}>
          ...
          <input type="file" accept="image/*" onChange={(e) => handleImageUpload(e, onUpload)} hidden />
        </label>
      </div>
  ...
  return (
    <div style={styles.container}>
            ...
            <div style={styles.tableWrapper}>
              <table style={styles.table}>
                ...
                <tbody>
                  {users.map(u => (
                    <tr key={u._id}>
                      <td><div style={{fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '10px'}}>{u.photo && u.photo.length > 50 ? <img src={u.photo} alt="" style={{width:'30px', height:'30px', borderRadius:'50%', objectFit:'cover'}}/> : <div style={{width:'30px', height:'30px', borderRadius:'50%', background:'#e2e8f0', display:'flex', alignItems:'center', justifyContent:'center', fontSize:'0.7rem'}}>{u.prenom[0]}</div>}{u.prenom} {u.nom}</div></td>
      ...
      {/* L'ÉDITEUR CMS D'UN CLUB SÉLECTIONNÉ */}
      {editingClub && (
        <div style={{background: 'white', borderRadius: '20px', padding: '30px', border: '1px solid #e2e8f0', boxShadow: '0 10px 30px rgba(0,0,0,0.05)'}}>
          ...
          <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '30px'}}>
            {/* Colonne Gauche: Infos & Président */}
            <div>
              ...
              <div style={{background: '#f8fafc', padding: '20px', borderRadius: '16px', border: '1px solid #e2e8f0', display: 'flex', gap: '20px', alignItems: 'flex-start'}}>
                <PhotoUpload 
                  photo={editingClub.president?.photo} 
                  label="Photo" 
                  onUpload={(base64) => setEditingClub({...editingClub, president: {...editingClub.president, photo: base64}})} 
                />
            ...
            {/* Colonne Droite: Staff & Sous-clubs */}
            <div>
              ...
              <div style={{display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '30px'}}>
                {editingClub.staff.map((member, i) => (
                  <div key={i} style={{display: 'flex', gap: '10px', alignItems: 'center', background: '#f8fafc', padding: '10px', borderRadius: '12px', border: '1px solid #e2e8f0'}}>
                    <PhotoUpload photo={member.photo} label="Photo" onUpload={(base64) => { const nS = [...editingClub.staff]; nS[i].photo = base64; setEditingClub({...editingClub, staff: nS}); }} />
                ...
                {editingClub.subClubs.map((sub, i) => (
                  <div key={i} style={{background: '#f8fafc', padding: '15px', borderRadius: '16px', border: '1px solid #e2e8f0', position: 'relative'}}>
                    ...
                    <div style={{borderTop: '1px dashed #cbd5e1', paddingTop: '10px', display: 'flex', gap: '10px', alignItems: 'center'}}>
                      <PhotoUpload photo={sub.responsable?.photo} label="Resp." onUpload={(base64) => { const nSC = [...editingClub.subClubs]; if(!nSC[i].responsable) nSC[i].responsable = {}; nSC[i].responsable.photo = base64; setEditingClub({...editingClub, subClubs: nSC}); }} />
                      <div style={{flex: 1, display: 'flex', flexDirection: 'column', gap: '5px'}}>
