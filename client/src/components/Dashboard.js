...
import ProfileModal from './ProfileModal'; // Assure-toi que ce fichier existe

export default function Dashboard({ token, user, onLogout, onUpdateUser }) {
  const [slots, setSlots] = useState([]);
  const [stats, setStats] = useState(null);
  const [showProfile, setShowProfile] = useState(false); // État pour le modal Profil
            ...
            {['Matin', 'Apr'].map(periodCode => {
              ...
              return (
                <div key={slot._id} className={`slot-card ${isFull ? 'full' : ''} ${isRegistered ? 'active' : ''}`}>
                  ...
                  <div className="avatars" style={{minHeight:'35px'}}>
                    <AnimatePresence>
                      {slot.ambassadors.map(amb => (
                        <motion.div 
                          ...
                          title={`${amb.prenom} ${amb.nom}`}
                        >
                          {amb.prenom[0]}{amb.nom[0]}
                        </motion.div>
                      ))}
                    </AnimatePresence>
                    {slot.ambassadors.length === 0 && <span style={{fontSize:'0.8rem', color:'#cbd5e1', fontStyle:'italic'}}>Libre</span>}
                  </div>
                  ...
                  {user.role === 'ambassadeur' ? (
                    ...
                    <div style={{
                      marginTop:'10px', padding:'8px', background:'rgba(255,255,255,0.5)', color:'#64748b', 
                      fontSize:'0.75rem', textAlign:'center', borderRadius:'6px', border:'1px dashed #cbd5e1', fontStyle:'italic'
                    }}>
                      Vue Administrateur
                    </div>
                  )}
